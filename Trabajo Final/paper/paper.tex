...
\IEEEPARstart{D}{esde} siempre las personas han expresado variados gustos y opiniones sobre distintos tópicos, siendo la literatura uno de los más discutidos. Los fundamentos de las opiniones pueden ser muy variados e incluso difusos para los dueños de las mismas, y muchas veces estar influenciadas por la opinion del resto de los usuarios\cite{muchnik} o ser poco objetivos debido a experiencias personales (la mayoria de los usuarios solo dan una reseña cuando su relacion con el producto es muy buena o muy mala\cite{hu}). 

A pesar de esto, los puntajes de reseña que los usuarios asignan en los sitios web tienden a converger a un número, mayormente positivo a medida que transcurre el tiempo\cite{zhang}; y aunque en general el comportamiento sigue siendo una incognita, hay estudios que afirman se trata de una distribucion binomial\cite{hu}. Sin embargo, no todo su comportamiento puede ser justificado por una tendencia\cite{zhang}, por lo que su información sigue siendo valiosa. 

En nuestro estudio intentamos responder la pregunta de si la legibilidad de los libros impacta de alguna forma en las reseñas que los usuarios otorgan. Suponemos que los libros más faciles de leer, es decir más legibles, tienden a tener mejor puntaje que aquellos que poseen una lectura mas dificultosa. Para demostrar nuestra hipotesis, recompilamos una amplia variedad de libros de distintos autores, géneros y formatos, junto con la información de las calificaciones asignadas por sus lectores, otorgada por la base de datos de Amazon\footnote{http://snap.stanford.edu/data/amazon-meta.html}, la cual fue usada en un análisis del marketing viral\cite{leskovec}. Intentamos que los libros analizados conformen un conjunto de datos lo mas homogeneo posible, para evitar sesgos ocasionados por una categoria u autor en particular.

Entendemos que a pesar del esfuerzo significativo del area linguistica para introducir criterios computacionales que modelen y evaluen la complejidad de un texto en el sentido de legibilidad, un esquema representativo y concluyente todavia esta faltando \cite{orlow, klare, kanungo, karmakar}. Por ello para medir la legibilidad utilizamos un conjunto de metricas clasicas, al contrario de una sola. De esta manera esperamos poder asegurar cierta consistencia en los resultados, que de lo contrario utilizando una sola metrica podria faltar ya que los resultados pueden no ser consistentes entre las mismas\cite{izgi}. Nos hubiese gustado utilizar métricas mas avanzadas durante la experimentación como \textit{Coh-Metrix}\cite{graesser}, que parecen tener una consistencia y utilidad mayores a las metricas básicas\cite{crossley}, pero no estaban disponibles para hacer de su uso fácilmente.
...
\subsubsection{ARI}
El índice de legibilidad automatizado (\textbf{ARI}) posee dos variables: caracteres por palabra y palabras por frase. El resultado corresponde al nivel de grado de los Estados Unidos. Su fórmula se da en la siguiente relación:
...
Consiste en una de las métricas más antiguas de legibilidad, comunmente utilizada en la academia e incorporada en la mayoría de los softwares de procesamiento de lenguaje. La métrica \textbf{Flesch-Kincaid Reading Ease} es el resultado de una fórmula matemática que incorpora el promedio de sílabas por palabra y el promedio de palabras por frase cada 100 palabras. Los resultados se miden en una escala del 1 al 100, donde 1 significa muy complicado de leer y 100 muy fácil.\\
...
A diferencia del resto de los tests de legibilidad, el \textbf{Coleman-Liau Index} se basa en la cantidad de caracteres por palabra en vez del número de sílabas para su cálculo. Devuelve el nivel de grado de los Estados Unidos del libro, valor entre 1 y 12.\\
...
A diferencia del resto de las métricas que usan la cantidad de caracteres o de sílabas en una palabra, \textbf{Dale-Chall} calcula el nivel de grado aproximado de un texto midiendo las ``palabras difíciles''. Para esto, se encuentra definida una lista de aproximadamente 3000 palabras conocidas por al menos el 80\% de los niños de quinto grado. Las palabras consideradas difíciles son las que no se encuentran en dicha lista. Cuanto más alto es el resultado, más alto es el nivel de grado del texto. 
