...
Las métricas de legibilidad son formulas que sirven para evaluar la legibilidad de un texto de forma automatica, normalmente utilizadas como reemplazo de una encuesta con personas. Las métricas analizadas en el presente trabajo fueron diseñadas entre los años 1940 a 1970, muchas veces teniendo en mente que deberian ser calculadas por maquinas simples o por personas. A pesar de haber pasado mucho tiempo desde su creación, estas métricas clásicas siguen siendo utilizadas en muchos estudios y hasta aparecen como condiciones necesarias legales en, por ejemplo, la creacion de las polizas de seguros automotrices. Las métricas estudiadas, exceptuando Flesch–Kincaid Reading Ease, simbolizan en sus valores la edad promedio necesaria o nivel educativo necesario de las personas para entender el texto analizado con la misma.

\subsection{ARI}

El índice de legibilidad automatizado (\textbf{ARI})\cite{ari-flesch} fue diseñado a pedido de la fuerza aerea de los estados unidos con el proposito de monitorear en tiempo real la legibilidad de los textos producidos por maquinas de escribir electricas. A diferencia de otros indices, junto con el Coleman-Liau, ARI se basa en un factor de carácteres por palabra, en vez del usual silabas por palabra. Aunque la opinion varia sobre la perdida de precision en el indice con uso de este factor, usualmente es mas rápido de calcular, ya que el numero de caracteres es mucho mas facil de contar de forma automatica que el numero de silabas\cite{liang}.
...
El indice \textbf{Coleman-Liau}\cite{coleman-liau} fue diseñado para ser facilmente calculable de forma mecanica desde texto impreso en papel. Como solamente requiere analizar los carácteres por palabra, teoricamente podria ser usado en conjunto con escaners mecanicos simples que solo necesiten reconocer caracteres, palabras y limites de las oraciones, eliminando la necesidad de un reconocimiento completo de caracteres. La fórmula del mismo se define como:

$$CLI = 5.88 \cdot \frac{letras}{palabras} - 29.6 \cdot \frac{frases}{palabras} - 15.8$$

\subsection{Flesch–Kincaid}
Las formulas de legibilidad \textbf{Flesch–Kincaid Reading Ease}\cite{ari-flesch} y \textbf{Flesch–Kincaid Grade Test} fueron diseñadas bajo el pedido de la marina de los Estados Unidos para evaluar la dificulad de los manuales tecnicos, antes de el Grade Test volverse un estandar en la milicia de los Estados Unidos. Ambas formulas se correlacionan casi de forma inversa, lo que tiene sentido ya que usan las mismas medidas (relacion entre silabas y palabras y la relacion entre palabras y oraciones), diferenciandose solo en sus factores.

En este trabajo analizaremos solamente la variante Flesch-Kincaid Reading Ease, ya que consiste en una de las métricas más antiguas de legibilidad, y es comúnmente utilizada en la academia e incorporada en la mayoría de los softwares de procesamiento de lenguaje. Los resultados de esta métrica, a diferencia del resto, se miden en una escala del 1 al 100, donde 1 significa muy complicado de leer y 100 muy fácil.\\
...
\subsection{SMOG}
La medida simple de Gobbledygook (\textbf{SMOG})\cite{smog} es una medida de legibilidad extensamente utilizada, particularmente en la verificacion de mensajes de salud\cite{hedman}. La formula fue desarrollada en 1969 como un substituto mas certero y mas facil de calcular del indice Gunning Fog, razón por la cual el analisis de esta ultima es omitido en el trabajo.
...
La fórmula de legibilidad Läsbarhetsindex (\textbf{LIX})\cite{lix-rix} fue desarrollada en Sueiza por Björnsoon. Lix es una fórmula de legibilidad poco conocida que es rápida de usar, confiable y fácil de interpretar. Fue especialmente diseñada para como un indice de legibilidad para textos de lenguajes extranjeros. En 1980 Jonathan Anderson la estudio y comprobó que funcionaba en Francés, Alemán, Griego e Inglés. También realizó una simplifcación de la misma llamandola RIX, la cúal no fue incluida en el estudio.

Ya que la fórmula es independiente del lenguaje, la misma se define en base de la cantidad de palabras, cantidad de períodos y cantidad de palabras largas (en ingles, de más de 6 letras). Se la define como:
...
En lo que sigue, se explican las consideraciones que tuvimos en cuenta a la hora de seleccionar los libros a analizar.
...
\section{Experimentos y Resultados}
 ...
 En primer lugar, estudiamos el conjunto de categorías presentadas por Amazon y las posibles reseñas (utilizando la escala del 0 al 5, saltando de a 0.5). Luego, decidimos dividir las categorías en cuatro clusters (numerados  del 1 al 4), y descargar 15 libros de cada reseña y cada cluster. Dado que no encontramos libros con reseñas del 0 al 1.5, y la cantidad de libros calificados con 1.5 era escasa, insertamos los libros de 1.5 a 2.5 en un mismo puntaje, y para el resto se usó la descripción anterior.\\
