...
%TODO: Agregar factor de normalizacion con funcion partida (If the percentage of difficult words is above 5%, then add 3.6365 to the raw score to get the adjusted score, otherwise the adjusted score is equal to the raw score.)
...
Una vez obtenidos los resultados, proseguimos a realizar un analisis preliminar utilizando boxplots. Como cada una de las métricas devuelve valores en escalas distintas, normalizamos los resultados de las mismas, de forma de poder comparar facilmente. En el caso de Flesch-Kincaid ademas de normalizar invertimos sus valores, de manera tal de poder evidenciar la misma relacion que en el resto de las metricas.
...
%TODO: Especificar grupos utilizados para el analisis
...
Si analizamos en detalle el boxplot generado en base a los valores devueltos por la metrica SMOG, podremos ver que los valores estan centrados alrededor del 8, lo que significa que una persona con 8 años de educacion deberia ser capaz de entender la mayoria de los libros. Utilizando un histograma para analizar la distribucion de los valores podremos ver que la gran parte de los libros toman valores entre 7 y 9, tendencia que se puede observar tambien en el resto de las metricas, evidenciando asi que gran parte de los libros utilizados para el analisis no son dificiles de leer. Por ello, es esperable que las diferencias en las medianas entre los dos grupos analizados no sean muy grandes.
...
Para dar algunos ejemplos concretos de los valores asignados de la metrica a libros, seleccionamos los libros con menor y mayor requerimiento de años y uno que este cercano a la media. El libro ``Day No Pigs Would Die'' por ``Robert Newton Peck'' requiere, segun la metrica, 5 años de educacion para ser leido y tiene un puntaje promedio de 3.5. Segun los comentarios que pueden ser leidos en la pagina de Amazon \footnote{https://www.amazon.com/dp/1883332052} aparenta ser un libro con una historia pobre y escrito de manera burda. Por otro lado, el libro ``A New Kind of Science'' por ``Stephen Wolfram'' requiere 15 años de educacion y tiene un puntaje promedio de 3. El libro, segun los comentarios\footnote{https://www.amazon.com/dp/1579550088}, trata sobre algunos temas de ciencias de la computacion, por lo que tiene sentido su lenguaje mas complicado. Sin embargo este libro le desagrada a muchos usuarios ya que trata como nuevos temas que son bien conocidos en el ambito y se extiende mucho sobre los mismos, probablemente por ignorancia del autor\footnote{https://www.amazon.com/review/R2NIU9I3JQZP5J}. Finalmente el libro ``Watership Down'' por ``Richard Adams requiere 7 años de educacion y tiene un puntaje promedio de 4.5. Este libro aparenta ser literatura clasica y posee muy buenas reseñas por parte de los usuarios\footnote{https://www.amazon.com/dp/1565115864}.


\section{Conclusiones y trabajo futuro} 

En el presente trabajo, evidenciamos que en lineas generales, existe una relacion entre el puntaje promedio de las reseñas y la legibilidad de un libro. Todas las metricas que utilizamos, en mayor o menor medida y con una acotada cantidad de libros para su analisis, presentaron evidencia de esta relacion. Es posible que con una mayor cantidad de libros y metricas mas avanzadas se pueda estudiar mas en profundidad el comportamiento de esta relacion y poder llegar a resultados mas concretos.

En un trabajo futuro seria interesante analizar el conjunto de libros con una metrica multi-variable con elementos de procesamiento de lenguaje natural, como lo es \textit{Coh-Metrix}\cite{graesser}. Este tipo de metricas describen mejor la forma en el que libro esta escrito y por lo tanto podria evidenciar mas facilmente la relacion. Tambien seria interesante repetir el analisis con mas y actualizada (ya que el dataset lleva 7 años) informacion, de forma tal de poder realizar analisis particulares para cada una de las categorias y observar si existe alguna variacion de los resultados en cada una de ellas.

Es importante remarcar, que durante la busqueda de alguno de los libros pudimos observar que en el transcurso de estos 7 años su puntaje aumento en Amazon. Como nombramos anteriormente, algunos estudios indican que este es un comportamiento esperado a lo largo de tiempo\cite{zhang}, por lo que si bien llamo nuestra atencion, no nos parecio preocupante.
