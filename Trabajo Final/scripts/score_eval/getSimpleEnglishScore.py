...
for i in range(0,len(wordBooks)):
	wordBooks[i] = re.sub("[^a-z]", "", wordBooks[i])
	if wordBooks[i] in dictionary:
		totalCounter +=1
		if wordBooks[i] in simpleDictionary:
			simpleCounter +=1
	#print (i/float(len(wordBooks)))*100,"%"

print "Relacion de cantidad: ", float(simpleCounter)/totalCounter*100, "%"

totalCounterVocabulary = 0
simpleCounterVocabulary = 0
notInVocabulary = 0

wordBooksSet = set(wordBooks)
for word in wordBooksSet:
	if word in dictionary:
		totalCounterVocabulary +=1
		if word in simpleDictionary:
			simpleCounterVocabulary +=1
	else:
		#print word
		notInVocabulary += 1

print "Relacion de vocabulario: ", (float(simpleCounterVocabulary)/totalCounterVocabulary)*100, "%"
